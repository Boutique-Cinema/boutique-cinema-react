...
export default function AdminReservationList() {
    ...
    const filteredBookings = bookings.filter((booking) => {
        return booking[selected].toString().toLowerCase().includes(searchTerm.toLowerCase());
    });
    ...
    return (
        <div style={{ padding: '20px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '20px' }}>
                <select onChange={handleSelect} value={selected} style={{ marginRight: '10px' }}>
                    {selectList.map((item) => (
                ...
                <tbody>
                    {filteredBookings.map((booking) => (
                        <tr key={booking.bookingId} style={{ cursor: 'pointer' }}>
                            ...
                            <td style={thTdStyle}>{booking.isRefunded ? '환불 완료' : '환불 불가'}</td>
